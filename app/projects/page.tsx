...
export default function ProjectsPage() {
  ...
  const categories = ['all', ...new Set(siteConfig.projects.map(p => p.category))]
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        >
          ...
          <p className="text-xl text-gray-400">
            Open-source tools, automation scripts, and creative coding.
          </p>
        </motion.div>
        ...
        >
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-grow">
              <input
                type="text"
                placeholder="Search projects (e.g. 'python', 'opencv')..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full px-4 py-3 bg-cyber-darker border border-cyber-border rounded-lg
                         text-gray-300 placeholder-gray-500 focus:outline-none focus:border-cyber-cyan
                         transition-colors"
              />
            </div>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-medium transition-all ${
                    selectedCategory === category
                      ...
                      : 'bg-cyber-border/50 text-gray-300 hover:bg-cyber-cyan/20 hover:text-cyber-cyan'
                  }`}
                >
